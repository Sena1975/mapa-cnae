...
class GeocodeStats extends Command
{
    ...
    public function handle()
    {
        ...
        $total = $conn->table('geocode_cache')->count();
        $ok    = $conn->table('geocode_cache')->whereNotNull('lat')->whereNotNull('lng')->count();
        $nulls = $total - $ok;
        ...
        $this->table(['input_address','formatted_address','lat','lng','updated_at'], $latest->toArray());
